...
# Función para la clasificación basada en la distancia euclidiana al cuadrado
def euclidean_distance_classification(X_train, y_train, X_test, threshold=None):
    y_pred = []
    confidences = []
...
for num_components in component_numbers:
    ...
    # Realizar predicciones en el conjunto de prueba con la distancia euclidiana al cuadrado
    threshold = 1000  # Ajustar según sea necesario
    y_pred_euclidean, confidences_euclidean = euclidean_distance_classification(lda_X_train, y_train, lda_X_test, threshold)

    # Filtrar predicciones None (umbral no alcanzado)
    y_pred_euclidean_filtered = [pred for pred in y_pred_euclidean if pred is not None]
    y_test_filtered = [true for pred, true in zip(y_pred_euclidean, y_test) if pred is not None]

    # Evaluar las predicciones de la distancia euclidiana al cuadrado
    accuracy_euclidean = accuracy_score(y_test_filtered, y_pred_euclidean_filtered)
    pca_accuracies.append(accuracy_euclidean)
...
for num_lda_components in lda_component_numbers:
    ...
    # Evaluar las predicciones de la distancia euclidiana al cuadrado
    accuracy_euclidean = accuracy_score(y_test_filtered, y_pred_euclidean_filtered)
    lda_accuracies.append(accuracy_euclidean)
