...
# Aplicar LDA
lda = LDA(n_components=14)  # min(len(np.unique(y_train)) - 1, num_components)
...
# Guardar las características LDA del conjunto de entrenamiento
with open('lda_X_train.pkl', 'wb') as f:
    pickle.dump(lda_X_train, f)
...
y_pred_cosine, confidences_cosine = cosine_distance_classification(lda_X_train, y_train, lda_X_test,
                                                                   threshold=threshold)
...
print(f'Accuracy on test set (Cosine Distance): {accuracy_cosine:.2f}')

print('Classification Report (Cosine Distance):')
print(classification_report(y_test_filtered, y_pred_cosine_filtered, zero_division=0))

print('Confusion Matrix (Cosine Distance):')
conf_matrix = confusion_matrix(y_test_filtered, y_pred_cosine_filtered)
print(conf_matrix)
...
plt.title('Confusion Matrix (Cosine Distance)')
...
results_df = pd.DataFrame({
    'True_Label': y_test,
    'Predicted_Label_Cosine': y_pred_cosine,
    'Confidence_Cosine': confidences_cosine
})
results_df.to_csv('predictions_results_cosine.csv', index=False)
'''
