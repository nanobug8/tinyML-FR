...
# Función de clasificación basada en la distancia de Mahalanobis
def mahalanobis_distance_classification(X_train, y_train, X_test, threshold=None):
    ...
    for test_point in X_test:
        distances = [mahalanobis(test_point, train_point, inv_covariance_matrix) for train_point in X_train]
        min_index = np.argmin(distances)
        confidence = 1 / (1 + distances[min_index]) if distances[min_index] != 0 else 1
...
# Aplicar PCA
num_components = 9  # Ajustar según sea necesario
...
# Evaluar el rendimiento con distancia de Mahalanobis
threshold = 0.5  # Ajustar según sea necesario
y_pred, confidences = mahalanobis_distance_classification(lda_X_train, y_train, lda_X_test, threshold)

accuracy = accuracy_score(y_test, y_pred)
print(f'Accuracy on test set (Mahalanobis Distance): {accuracy:.2f}')
...
plt.ylabel('Actual')
plt.title('Confusion Matrix (Mahalanobis Distance)')
plt.show()
...
results_df.to_csv('predictions_results_mahalanobis.csv', index=False)
